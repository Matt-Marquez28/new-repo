...
export const recommendCandidates = async (req, res) => {
  ...
  try {
    // Sanitize inputs
    const sanitizedEducationalLevels = educationalLevels?.length
      ? educationalLevels
      : [];
    const sanitizedSkills = skills?.length ? skills : [];
    const sanitizedSpecializations = specializations?.length
      ? specializations
      : [];
    ...
    const pipeline = [
      {
        $search: {
          ...
          compound: {
            should: [
              // All conditions are now optional (OR)
              ...(sanitizedSpecializations.length
                ? [
                    {
                      text: {
                        query: sanitizedSpecializations,
                        ...
                        score: { boost: { value: 3 } }, // Keep the higher boost for specializations
                      },
                    },
                  ]
                : []),
              ...
              ...(sanitizedSkills.length
                ...
                : []),
              ...(sanitizedEducationalLevels.length
                ? [
                    {
                      text: {
                        query: sanitizedEducationalLevels,
